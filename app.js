 ...
 case "Strong Tea":
  ...
  case "Light Tea":
   ...
   var cups = parseInt(document.getElementById("cups").value) || 1;
    ...
    var stockElements= document.querySelectorAll(".stock-elements > p");
   var stockObject = {};
   stockElements.forEach((stockElement, index)=>{
    if(index %2==0 ) {
        stockObject[stockElement.innerText.toLowerCase()] = parseInt(stockElements[index+1].innerText);
    }
   })
     var subtractedStock = {};
      Object.keys(stockObject).forEach((key,index)=>{
         subtractedStock[key] = stockObject[key] - Result[key];
         stockElements[index * 2+1].innerText = String(stockObject[key] - Result[key] *cups);
